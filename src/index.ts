...
import { ResyScheduler } from './scheduler';
...
program
  ...
  .action(async (options) => {
    ...
    if (options.venue && options.date) {
      ...
      const slots = await client.findAvailableSlots(options.venue, options.date, partySize);
      ...
      for (const venue of config.venues) {
        ...
        const slots = await client.findAvailableSlots(venue.id, venue.date, partySize);
...
program
  ...
  .action(async (options) => {
    ...
    const venues = options.venue && options.date
      ? [{ id: options.venue, name: `Venue ${options.venue}`, date: options.date, partySize: options.partySize, preferredTimes: options.time ? [options.time] : undefined }]
      : config.venues;
    ...
    for (const venue of venues) {
      ...
      console.log(chalk.cyan(`  Found slot: ${slot.time}`));
